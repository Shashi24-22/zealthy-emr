...
export async function POST() {
  ...
  const mark = await db.user.create({ data: { name: 'Mark Johnson', email: 'mark@example.com', password } });
  const lisa = await db.user.create({ data: { name: 'Lisa Smith',   email: 'lisa@example.com', password } });
